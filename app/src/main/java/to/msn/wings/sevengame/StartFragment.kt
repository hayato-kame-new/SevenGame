            ...
            */
            ...
            if (subListComA.size != 0) {  // Aは　出せるので出す
                ...
                if (_comAPassCounter <= -1 && _comBPassCounter <= -1 ) { // ゲーム終了
                    ...
                    // 上で置いたカードを trueにしていますから

                    // ここではもう何も書かなくていいです！！！！

                    // ここもう一つ処理を書く必要がある
                    // 出したカードが １ や 13　ならば、気を付ける 次に置けるカードが変化する １ や 13　だった時にだけする処理
                    // 1を出した時に、逆向きになっていた場合だけ考える つまり、8 9 10 11  となっていた時に自分が  13 を出したとする
                    // 次の人が １２を出した時にはどうする?
                    // 13　placed trueにします  12を出した時に 8 9   12 13 となっていたとする 11を出した時にどうします?
                    // つまり、 8 ~ 12 を出したとしても  ８９１０1112 13 のplacedが カウント 5かどうかを調べないといけないんです

                    //  2 ~ 6 を出したとしても 1 2 3 4 5 6 の中で カウント ５だったら 分岐するんです
